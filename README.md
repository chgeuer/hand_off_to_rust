...
```
  Client (ncat / telnet)          Elixir (BEAM)               Rust process
  ┌──────────────────┐    TCP    ┌────────────────┐          ┌──────────────┐
  │                  │◄─────────►│  Listener      │          │              │
  │  ← hello from    │           │  (GenServer)   │  UDS +   │              │
  │    elixir (1..10)│           │                │SCM_RIGHTS│  rust_handler│
  │                  │           │  sends FD ────►├─────────►│              │
  │  ← Hello from    │◄──────────┤                │          │  takes over  │
  │    Rust          │           └────────────────┘          │  the socket  │
  │                  │◄─────────────────────────────────────►│              │
  │  → ping 1        │           (Elixir is done)            │  ← echo loop │
  │  ← Rust echo:    │                                       │              │
  │    ping 1        │                                       │  exits on    │
  │                  │                                       │  client EOF  │
  └──────────────────┘                                       └──────────────┘
...
The key mechanism is **`SCM_RIGHTS`** — a POSIX feature that lets one process send a file descriptor to another process over a Unix domain socket. The kernel creates a new FD in the receiver that points to the same underlying socket object.
...
See [`docs/how-it-works.md`](docs/how-it-works.md) for the full deep dive including gotchas around non-blocking sockets, `CLOEXEC`, and why you must *not* call `gen_tcp.close/1` after handoff.
